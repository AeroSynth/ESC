...
Note that artifacts on left side of vertical line appear when you switch screens using Alt-Tab
...
device = 11#6#0 # id of the audio device by default 12 for grey box, 11 for Focusrite
window = 32#128#512 #1000# window for the data.  Same as time base
...
samplerate= 22050#44100#11025#22050
length  = int(window*samplerate/(1000*downsample))
length = length-1 #seems to remove artifact on left side of vertical center line
...
#set vertical range
sizeD = .6
ax.set_ylim(bottom=-sizeD,top=sizeD)
bx.set_ylim(bottom=-sizeD,top=sizeD)
...
def update_plot(frame):
    ...
    #set DC offsets for each channel
    left_offset = 0.015
    right_offset = - 0.102
    ...
    if plot_side==False:
        ...
        while True:
            ...
            shift = len(data1)
            data1 = data1 + right_offset
            #print(shift)
            plotdata = np.roll(plotdata, -shift,axis = 0)
        # Elements that roll beyond the last position are 
        # re-introduced 
        #while True:
            plotdata[-shift:,:] = data1
            #plotdata[-1:]=0
